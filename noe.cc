/*
 * calculate NOEs from MD trajectory
 *
 */
...
const char *usage = "\n\n"
"        Usage\n"
"             noe -d protonpairs.dat -o outp_root.dat -s xyz.pdb trj1 trj2 ...trjN\n\n"
"        ./rdc -d protonpairs.dat -o outp_root.dat -s xyz.pdb trj1 ... trjN\n"
"             will read the list of data in protonpairs.dat and write to "
"		outp_root.dat\n"
"\n\n";
...
int main(int argc, char **argv)
{
	...
	int *sel, nsel;
	*/
	string itrj,noe_pairs,output,pdbfile;
	FILE *outp;
	coords *crd;
	bool align;
	...
	for (int i=0; i<nrdc_fit; i++) {
		fprintf(stdout,"FIT DATA: %5i %4s %5i -- %5i %4s %5i : %8.3f (%12.3f)\n",
	...
	for (int i=0; i<nrdc_bc; i++) {
		fprintf(stdout,"BC DATA: %5i %4s %5i -- %5i %4s %5i : %8.3f (%12.3f)\n",
				bc_data[i].resi,bc_data[i].atomi.c_str(),bc_data[i].indi,
				bc_data[i].resj,bc_data[i].atomj.c_str(),bc_data[i].indj,
				bc_data[i].Dij,bc_data[i].Dmax);
	}
	//fprintf(stdout,"GOT HERE 00\n");
	//fflush(stdout);
	rdc_vec = gsl_vector_alloc(nrdc_fit);
	rdc_vec_bc_unnorm = gsl_vector_alloc(nrdc_bc);
	bc = gsl_vector_alloc(nrdc_bc);
	bc_unnorm = gsl_vector_alloc(nrdc_bc);
	...
	if (ntrj==0) {
		...
		if (rdc_bc == rdc_fit) {
			...
			rdc_vec_bc = gsl_vector_alloc(nrdc_bc);
			fill_rdcvec(bc_data, rdc_vec_bc);
			//coef_mat_bc = gsl_matrix_alloc(nrdc_fit,5);
			coef_mat_bc = gsl_matrix_alloc(nrdc_bc,5);
			gsl_matrix_set_zero(coef_mat_bc);
			calc_coef_trj(*crd, trjfiles, bc_data, coef_mat_bc, sel, nsel, weight_vec);
		//	calc_coef_pdb(*crd,bc_data,coef_mat_bc);
		}
		fprintf(stderr,"got here 1\n");
	}
	//fprintf(stdout,"GOT HERE 02\n");
	//fflush(stdout);
	/*
	for (int i=0; i<nrdc_fit; i++) {
