...
def calc_coef_nmr(nmr_file, rdc_list,refpdb):
	...
	nrdc = len(rdc_list)
	coef_mat = numarray.zeros([nrdc,5],numarray.Float64)
	rdc_vec = numarray.array(map(lambda x: x[4]/x[5],rdc_list),numarray.Float64)
...
solution = numarray.linear_algebra.linear_least_squares(coef_mat_fit,rdc_vec_fit)
...
bc = numarray.matrixmultiply(coef_mat_bc,S)
